...
import com.gilbram.whatsapp.util.Message
...
class ConversationAdapter (private val messages:ArrayList<Message>, val userId:String?):
    ...
    class ConversationViewHolder(val view: View) : RecyclerView.ViewHolder(view) {
        fun bindItem(message: Message) { view.findViewById<TextView>(R.id.txt_message).text = message.message
        }
    }
    ...
    override fun getItemViewType(position: Int): Int { if (messages[position].sendBy.equals(userId)){
        return MESSAGE_CURRENT_USER } else {
